...
impl Path {
    ...
    // pub(crate) fn stroke(&self, transform: Mat2x2, width: f32) -> Polygon {
    //     let polygon = self.flatten(transform);

    //     let mut contours = Vec::new();
    //     let mut points = Vec::new();

    //     for contour in 0..polygon.contours.len() {
    //         let stroke_contour = points.len();
    //         contours.push(stroke_contour);
    ...
    //         points.extend(polygon.points[start..end].iter().rev());

    //         for i in 0..len {
    //             let prev = points[(i + points.len() - 1) % (end - start)];
    //             let curr = points[i];
    //             let next = points[(i + 1) % (end - start)];
